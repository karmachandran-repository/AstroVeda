/**
 * AstroVeda Book Ingestion & Structured Knowledge Base Engine
 * Extracts text from PDF books and indexes them by planet, sign, house, and yoga
 * to dynamically expand the predictive capability of the software.
 */

const fs = require("fs");
const path = require("path");
// pdf-parse will be loaded dynamically so that the server starts even if npm is building
let pdfParse;
try {
  pdfParse = require("pdf-parse");
} catch (e) {
  console.warn("pdf-parse not yet installed, dynamic uploads will be ready after npm install is completed.");
}

const DB_PATH = path.join(__dirname, "..", "data", "knowledge_db.json");

// Foundational classical ruleset ( Brihat Parashara Hora Shastra & Phaladeepika )
...
function initDB() {
  ...
  if (!fs.existsSync(DB_PATH)) {
    fs.writeFileSync(DB_PATH, JSON.stringify(DEFAULT_KNOWLEDGE, null, 2), "utf8");
  }
}
...
function loadKnowledgeBase() {
  ...
  try {
    ...
    console.error("Error reading knowledge database, using defaults:", e);
...
// Parses planetary, house, and yoga details out of a paragraph of text
function extractAstrologicalRules(text, bookTitle) {
  ...
  sentences.forEach(sentence => {
    ...
    planets.forEach(p => {
      houses.forEach(h => {
        // Regex checking for planet and house nearby in sentence
        const regex = new RegExp(`${p}.{1,40}${h.name}|${h.name}.{1,40}${p}`, "i");
        if (regex.test(sentence)) {
          const key = `${p}_${h.key}`;
          if (!kb.planets_in_houses[key]) {
            kb.planets_in_houses[key] = [];
          }
          // Avoid duplicate rules
          if (!kb.planets_in_houses[key].includes(sentence)) {
            kb.planets_in_houses[key].push(`${sentence} (Source: ${bookTitle})`);
            rulesIngested++;
          }
        }
      });
    ...
    planets.forEach(p => {
      signs.forEach(s => {
        const regex = new RegExp(`${p}.{1,30}in\\s+${s}`, "i");
        if (regex.test(sentence)) {
          const key = `${p}_${s}`;
          if (!kb.planets_in_signs[key]) {
            kb.planets_in_signs[key] = [];
          }
          if (!kb.planets_in_signs[key].includes(sentence)) {
            kb.planets_in_signs[key].push(`${sentence} (Source: ${bookTitle})`);
            rulesIngested++;
          }
        }
      });
    ...
    // 3. Scan for Yogas
    yogas.forEach(y => {
      const regex = new RegExp(y.name, "i");
      if (regex.test(sentence)) {
        const key = y.key;
        if (!kb.yogas[key]) {
          kb.yogas[key] = [];
        }
        if (!kb.yogas[key].includes(sentence)) {
          kb.yogas[key].push(`${sentence} (Source: ${bookTitle})`);
          rulesIngested++;
        }
      }
    });
  ...
  // Track book metadata
  if (!kb.book_meta) kb.book_meta = [];
  const existingBook = kb.book_meta.find(b => b.title === bookTitle);
  if (existingBook) {
    existingBook.rulesCount += rulesIngested;
    existingBook.dateIngested = new Date();
  } else {
    kb.book_meta.push({
      title: bookTitle,
      rulesCount: rulesIngested,
      dateIngested: new Date()
    });
  }

  saveKnowledgeBase(kb);
  return rulesIngested;
}

// Ingest a PDF file
async function ingestBookPDF(filePath, customTitle) {
  if (!pdfParse) {
    throw new Error("pdf-parse dependency is loading. Please try again in a few seconds.");
  }
  const title = customTitle || path.basename(filePath, ".pdf");
  ...
  try {
    ...
    // Ingest the text to identify astrological rules
    const rulesCount = extractAstrologicalRules(text, title);
    
    return {
      success: true,
      title,
      pages: parsedData.numpages,
      rulesIngested: rulesCount
    };
  } catch (err) {
